...
const WEATHER: InferencePromptOption[] = [
  ...
  },
  {
    id: 'dense-fog',
    label: '浓雾',
    prompt: '浓厚低空雾幕和前景乳白色雾层覆盖画面约75%至90%，雾层从画面四周和前景压入，仅保留目标周围约10%至25%的近景地面窗口；远处跑道、滑行道和背景设施大范围消隐为低对比度轮廓，画面大半部分被有层次的半透明浓雾遮盖而不是均匀白色滤镜；目标被雾部分遮挡，仅保留约30%至50%对比度的主要轮廓和可辨外形，不被完全抹除；雾层不改变严格正射视角，不出现地平线和天空；无雨丝、无降雪、无明显积水和强烈阳光',
  },
];
