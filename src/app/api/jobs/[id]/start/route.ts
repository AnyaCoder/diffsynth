...
import { queueTrainJob, startJobNow } from '@/server/jobLifecycle';
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    await recordAuditEvent(request, {
      ...
      resourceType: 'job',
      statusCode: 500,
      detail: { error: error?.message || 'Failed to start job', job_type: job.job_type, name: job.name },
    });
