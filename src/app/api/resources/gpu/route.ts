...
async function getMacGpuInfo(): Promise<MacGpuResult | null> {
  try {
    ...
    try {
      const spOut = execSync('system_profiler SPDisplaysDataType 2>/dev/null | grep -E "Chipset Model|Total Number of Cores"', { encoding: 'utf-8', timeout: 5000 });
      ...
      if (nameMatch) {
        ...
        if (coresMatch) gpuName += ` GPU (${coresMatch[1]} cores)`;
      }
    } catch {}
    ...
    try {
      ...
      try {
        ...
        if (fanKeys.length > 0) fanSpeed = fanData[fanKeys[0]].rpm || 0;
      } catch {}
      try {
...
async function getGpuStats() {
  ...
  const { stdout } = await execAsync(command, { env: { ...process.env, CUDA_DEVICE_ORDER: 'PCI_BUS_ID' } });
  return stdout
    .trim()
    .split('\n')
    .filter(Boolean)
    .map(line => {
      const [index, name, driverVersion, temperature, gpuUtil, memoryUtil, memoryTotal, memoryFree, memoryUsed, powerDraw, powerLimit, clockGraphics, clockMemory, fanSpeed] = line
        .split(', ')
        .map(item => item.trim());
      ...
      return {
        ...
        clocks: {
          graphics: parseInt(clockGraphics, 10),
          memory: parseInt(clockMemory, 10),
        },
        fan: {
          speed: parseInt(fanSpeed, 10) || 0,
        },
      };
    });
}
...
export async function GET() {
  try {
    ...
    if (isMac) {
      ...
      return NextResponse.json({ hasNvidiaSmi: false, isMac: true, gpus: [], error: 'Could not read Mac GPU stats' });
    }

    const hasNvidiaSmi = await checkNvidiaSmi(isWindows);
    if (!hasNvidiaSmi) {
      return NextResponse.json({ hasNvidiaSmi: false, isMac: false, gpus: [], error: 'nvidia-smi not found or not accessible' });
    }
    ...
    return NextResponse.json({ hasNvidiaSmi: false, isMac: false, gpus: [], error: error?.message || 'Failed to fetch GPU stats' }, { status: 500 });
  }
}
